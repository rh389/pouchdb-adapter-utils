...
function parseRevisionInfo(rev) {
  if (!/^\d+-./.test(rev)) {
    return (0, _pouchdbErrors.createError)(_pouchdbErrors.INVALID_REV);
  }
  var idx = rev.indexOf('-');
...
function parseDoc(doc, newEdits) {
  ...
  if (newEdits) {
    if (!doc._id) {
      doc._id = (0, _pouchdbUtils.uuid)();
    }
    newRevId = doc.__newRevHash || (0, _pouchdbUtils.rev)();
  ...
  (0, _pouchdbUtils.invalidIdError)(doc._id);
  ...
  for (var key in doc) {
    ...
    if (Object.prototype.hasOwnProperty.call(doc, key)) {
      ...
      if (specialKey && !reservedWords[key]) {
        var error = (0, _pouchdbErrors.createError)(_pouchdbErrors.DOC_VALIDATION, key);
